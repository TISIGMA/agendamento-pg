...
try {
    ...
    if (empty($sqlComandos)) {
        echo "Nenhuma diferença encontrada!\n";
        exit;
    }

    $arquivoSql = __DIR__ . '/alteracoes_banco.sql';
    file_put_contents($arquivoSql, implode("\n\n", $sqlComandos));
    echo "\nArquivo SQL gerado em: $arquivoSql\n";

    echo "\n=========================================\n";
    echo "Deseja executar as alterações? (s/n): ";
